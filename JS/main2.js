...
function calcula(data) {
  ...
  var checkedBoxes = Array.from(
    document.querySelectorAll("input[name = selParty]:checked")
  ).map(elt => elt.value);
  ...
  var tempMissed = members.filter(
    obj => obj.missed_votes_pct >= valMin && obj.missed_votes_pct <= valMax
  );
  ...
  var tempMissed2 = members.filter(
    obj => obj.missed_votes_pct >= valMin && obj.missed_votes_pct <= valMax
  );
  ...
  var tempLoyal = members.filter(
    obj =>
      obj.votes_with_party_pct >= valMin && obj.votes_with_party_pct <= valMax
  );
...
function isIncluded(x, lst, p, c) {
  return (
    (c.indexOf(p) != -1 && lst.length == 0) ||
    (c.indexOf(p) != -1 && lst.indexOf(x) != -1)
  );
}
